...
export function TasksPage() {
    ...
    if (isError) {
        return (_jsx(EmptyState, { title: t('common.errorTitle'), description: t('common.errorDescription'), action: { label: t('common.retry'), onClick: () => window.location.reload() }, icon: _jsxs("svg", { width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.5, children: [_jsx("circle", { cx: "12", cy: "12", r: "10" }), _jsx("path", { d: "M12 8v4M12 16h.01" })] }) }));
    }
    return (_jsxs(_Fragment, { children: [_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: 20 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' }, children: [_jsxs("div", { children: [_jsx("h2", { style: { fontSize: 'var(--text-2xl)', fontWeight: 700, color: 'var(--color-text-primary)' }, children: t('tasks.title') }), _jsxs("p", { style: { fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)', marginTop: 4 }, children: [total, " ", t('tasks.total'), overdueTasks.length > 0 && (_jsxs(_Fragment, { children: [' · ', _jsxs("span", { style: { color: 'var(--color-danger)', fontWeight: 500 }, children: [overdueTasks.length, " ", t('tasks.overdue')] })] }))] })] }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 10 }, children: [_jsx("div", { style: {
                                            display: 'flex',
                            ...
                            borderRadius: 'var(--radius)',
                            fontSize: 'var(--text-sm)',
                        }, children: [_jsxs("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "var(--color-danger)", strokeWidth: 2, children: [_jsx("circle", { cx: "12", cy: "12", r: "10" }), _jsx("path", { d: "M12 8v4M12 16h.01" })] }), _jsxs("span", { style: { color: 'var(--color-danger)', fontWeight: 500 }, children: [overdueTasks.length, " ", t('tasks.overdueAlert')] })] })), tasks.length === 0 ? (_jsx(EmptyState, { title: t('tasks.emptyState.title'), description: t('tasks.emptyState.description'), action: can('task:create')
                            ? { label: t('tasks.create'), onClick: () => setShowCreateModal(true) }
                            : undefined, icon: _jsxs("svg", { width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.5, children: [_jsx("path", { d: "M9 11l3 3L22 4" }), _jsx("path", { d: "M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" })] }) })) : view === 'board' ? (_jsx(BoardView, { tasks: tasks })) : (_jsx(ListView, { tasks: tasks }))] }), _jsx(CreateTaskModal, { isOpen: showCreateModal, onClose: () => setShowCreateModal(false) })] }));
}
// ─── Board view ───────────────────────────────────────────────────────────────
function BoardView({ tasks }) {
    ...
    return (_jsx("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, alignItems: 'start' }, children: STATUS_COLUMNS.map((col) => {
            ...
            return (_jsxs("div", { children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, padding: '0 4px' }, children: [_jsx("span", { style: { width: 8, height: 8, borderRadius: '50%', background: col.color, flexShrink: 0 } }), _jsx("span", { style: { fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text-primary)' }, children: col.label }), _jsx("span", { style: {
                                ...
                                color: 'var(--color-text-muted)',
                                fontSize: 'var(--text-sm)',
                            }, children: t('tasks.empty') })) : (colTasks.map((task) => (_jsx(TaskCard, { task: task, canChangeStatus: canChangeStatus }, task.id)))) })] }, col.id));
        }) }));
}
// ─── Task card (board) ────────────────────────────────────────────────────────
function TaskCard({ task, canChangeStatus }) {
    ...
    return (_jsxs("div", { style: {
            ...
            transform: hovered ? 'translateY(-1px)' : 'none',
                                            ...
                                            overflow: 'hidden',
                                            minWidth: 160,
                                        }, children: [_jsx("p", { style: { padding: '6px 10px 4px', fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }, children: "Move to" }), STATUS_COLUMNS.filter((col) => col.id !== task.status).map((col) => (_jsxs("button", { onClick: (e) => {
                                                    e.stopPropagation();
                                                ...
                                                }, onMouseEnter: (e) => (e.currentTarget.style.background = 'var(--color-bg-subtle)'), onMouseLeave: (e) => (e.currentTarget.style.background = 'transparent'), children: [_jsx("span", { style: { width: 7, height: 7, borderRadius: '50%', background: col.color, flexShrink: 0 } }), col.label] }, col.id)))] }))] })] })] }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' }, children: [_jsx(Avatar, { name: task.assignee, size: "xs" }), _jsxs("span", { style: {
                            fontSize: 'var(--text-xs)',
                    ...
                    lineHeight: 1.6,
                    zIndex: 1,
                }, children: ["\u2192 ", STATUS_COLUMNS.find((c) => c.id === nextStatus)?.label] }))] }));
}
// ─── List view ────────────────────────────────────────────────────────────────
...
function ListRow({ task, canChangeStatus, onStatusChange, }) {
    ...
    return (_jsxs("tr", { style: {
                ...
                task.overdue ? 'var(--color-danger-subtle)' : 'transparent';
        }, children: [_jsxs("td", { style: { padding: '12px 16px' }, children: [_jsx("p", { style: { fontSize: 'var(--text-sm)', fontWeight: 500, color: 'var(--color-text-primary)' }, children: task.title }), task.description && (_jsx("p", { style: { fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 280 }, children: task.description }))] }), _jsx("td", { style: { padding: '12px 16px' }, children: _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: 8 }, children: [_jsx(Avatar, { name: task.assignee, size: "xs" }), _jsx("span", { style: { fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)' }, children: task.assignee })] }) }), _jsx("td", { style: { padding: '12px 16px' }, children: _jsx(Badge, { variant: PRIORITY_VARIANT[task.priority], children: task.priority }) }), _jsx("td", { style: { padding: '12px 16px' }, children: _jsxs("span", { style: {
                        fontSize: 'var(--text-sm)',
                        ...
                        fontWeight: task.overdue ? 600 : 400,
                                ...
                                minWidth: 160,
                                    ...
                                    textAlign: 'left',
                                    transition: 'background var(--transition-fast)',
                                }, onMouseEnter: (e) => (e.currentTarget.style.background = 'var(--color-bg-subtle)'), onMouseLeave: (e) => (e.currentTarget.style.background = 'transparent'), children: [_jsx("span", { style: { width: 7, height: 7, borderRadius: '50%', background: col.color, flexShrink: 0 } }), col.label] }, col.id))) }))] }) })] }));
}
function StatusBadge({ status }) {
    ...
    return _jsx(Badge, { variant: variant, dot: true, children: label });
}
