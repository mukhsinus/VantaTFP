...
export function EmployeesPage() {
    ...
    const canAddEmployeeByRole = String(currentRole ?? '').toUpperCase() === 'ADMIN' || String(currentRole ?? '').toUpperCase() === 'OWNER';
    const { data: billing } = useBilling({ enabled: canAddEmployeeByRole });
    const [search, setSearch] = useState('');
    const [roleFilter, setRoleFilter] = useState('ALL');
    const [showCreateModal, setShowCreateModal] = useState(false);
    const [roleSheetEmployee, setRoleSheetEmployee] = useState(null);
    ...
    const isEmployeeLimitReached = Boolean(billing &&
    ...
    if (isLoading)
        return _jsx(EmployeesListSkeleton, {});
    if (isError) {
                    ...
                    paddingBottom: showFab
                        ...
                        : undefined,
                            ...
                            flexDirection: isMobile ? 'column' : 'row',
                                        ...
                                        }, children: t('employees.title') }), _jsxs("p", { style: { fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)', marginTop: 4 }, children: [employees.length, " ", t('employees.total')] })] }), showAddEmployeeButton ? (_jsx(Button, { variant: "primary", size: isMobile ? 'lg' : 'sm', onClick: () => setShowCreateModal(true), disabled: disableAddEmployee, title: disableAddEmployee ? t('employees.limitReached') : undefined, style: isMobile ? { width: '100%' } : undefined, children: "+ ".concat(t('employees.add')) })) : null] }), _jsxs("div", { style: { display: 'flex', gap: 10, alignItems: 'stretch', flexDirection: isMobile ? 'column' : 'row' }, children: [_jsx("div", { style: { flex: 1, maxWidth: isMobile ? '100%' : 300, width: '100%' }, children: _jsx(Input, { placeholder: t('common.search'), value: search, onChange: (e) => setSearch(e.target.value), leftIcon: _jsxs("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, children: [_jsx("circle", { cx: "11", cy: "11", r: "8" }), _jsx("path", { d: "M21 21l-4.35-4.35" })] }) }) }), _jsx("div", { style: {
                                    display: 'flex',
                                    gap: 8,
                                    width: isMobile ? '100%' : undefined,
                                    overflowX: isMobile ? 'auto' : undefined,
                                    paddingBottom: 2,
                                }, children: ['ALL', 'owner', 'manager', 'employee'].map((r) => (_jsx("button", { type: "button", onClick: () => setRoleFilter(r), style: {
                                        padding: '10px 16px',
                                        minHeight: 44,
                                        ...
                                        flexShrink: 0,
                                    }, children: r === 'ALL' ? t('employees.filters.all') : t(`employees.roles.${r}`) }, r))) })] }), filtered.length === 0 ? (_jsx(EmptyState, { title: t('employees.empty.title'), description: t('employees.empty.description'), action: showAddEmployeeButton && !disableAddEmployee
                            ? { label: "+ ".concat(t('employees.add')), onClick: () => setShowCreateModal(true) }
                            : undefined })) : (_jsx("div", { style: {
                            display: 'grid',
                            gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(280px, 1fr))',
                            gap: isMobile ? 10 : 16,
                        }, children: filtered.map((employee) => (_jsx(EmployeeCard, { employee: employee, currentUserId: currentUser?.userId ?? null, onOpenRoleSheet: setRoleSheetEmployee, onRequestRemoval: (emp, variant) => setConfirmRemoval({ employee: emp, variant }) }, employee.id))) }))] }), _jsx(EmployeesInviteFab, { visible: showFab, onInvite: () => setShowCreateModal(true) }), _jsx(RoleChangeBottomSheet, { employee: roleSheetEmployee, onClose: () => setRoleSheetEmployee(null) }), _jsx(ConfirmMemberSheet, { target: confirmRemoval, onClose: () => setConfirmRemoval(null), onConfirm: handleConfirmRemoval, isPending: isRemoving }), currentRole ? (_jsx(CreateUserModal, { isOpen: showCreateModal, onClose: () => setShowCreateModal(false), creatorRole: currentRole })) : null] }));
}
