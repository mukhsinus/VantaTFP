...
export function useUpdateTaskStatus() {
    ...
    const mutation = useMutation({
        ...
        onError: (error, variables, context) => {
            ...
            const message = error instanceof ApiError ? error.message : i18n.t('errors.generic.unexpected');
            toast.error(i18n.t('errors.task.updateStatusFailed'), variables.taskTitle ? `"${variables.taskTitle}": ${message}` : message);
        },
        onSettled: () => {
